        ...
        <div class="flex items-center justify-between mb-8">
            <div class="flex items-center gap-4">
                <a href="{{ route('admin.custom-requests.index') }}"
                    class="p-2 rounded-full bg-white text-gray-500 hover:text-indigo-600 hover:bg-gray-50 border border-gray-200 transition-all shadow-sm">
                ...
                <div>
                    <h1 class="text-2xl font-bold text-gray-900">Custom Request #{{ $custom->id }}</h1>
                    <p class="text-sm text-gray-500">Received on {{ $custom->created_at->format('M d, Y') }} at
                        {{ $custom->created_at->format('h:i A') }}</p>
                </div>
            </div>
            ...
            <form id="delete-form-{{ $custom->id }}" action="{{ route('admin.custom-requests.destroy', $custom->id) }}"
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            <div class="lg:col-span-2 space-y-6">
                ...
                <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6 md:p-8">
                    ...
                    <div
                        class="bg-gray-50 rounded-lg p-6 text-gray-700 leading-relaxed whitespace-pre-line border border-gray-100">
                        {{ $custom->message }}
                    </div>
                </div>
                ...
                <div
                    ...
                    <div class="mb-4">
                        ...
                        <p class="text-2xl font-mono font-bold text-purple-400 tracking-wider">
                            {{ $custom->product?->product_code ?? 'N/A' }}
                        </p>
                    </div>
                    ...
                    <div>
                        ...
                        <p class="font-medium text-gray-100">
                            {{ $custom->product?->name ?? 'Product Not Found' }}
                        </p>
                    </div>
                </div>
                {{-- Attachment Card --}}
                <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
                    ...
                    @if($custom->attachment)
                        <div
                            class="flex items-center justify-between p-4 bg-indigo-50 border border-indigo-100 rounded-lg group hover:border-indigo-200 transition-all">
                            <div class="flex items-center gap-4">
                            ...
                            <a href="{{ asset('storage/' . $custom->attachment) }}" download
                                class="px-4 py-2 bg-indigo-600 text-white text-sm font-medium rounded-lg hover:bg-indigo-700 shadow-md shadow-indigo-200 transition-all">
                                Download
                            </a>
                        </div>
                    @else
                ...
                <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
                    ...
                    <form action="{{ route('admin.custom-requests.update-status', $custom->id) }}" method="POST">
                        ...
                        <div class="relative">
                            <select name="status" onchange="this.form.submit()"
                                ...
                                <option value="pending" {{ $custom->status == 'pending' ? 'selected' : '' }}>🟡 Pending
                                    Processing</option>
                                <option value="approved" {{ $custom->status == 'approved' ? 'selected' : '' }}>🟢 Approved
                                </option>
                                <option value="delivered" {{ $custom->status == 'delivered' ? 'selected' : '' }}>🔵 Delivered
                                </option>
                            </select>
                            <div
                    ...
                    <div class="mt-4 pt-4 border-t border-gray-100">
                        <div class="flex items-center justify-between text-sm">
                            ...
                            <span class="font-bold text-gray-900 bg-gray-100 px-2 py-1 rounded">{{ $custom->quantity }}
                                Units</span>
                        </div>
                    </div>
                </div>
